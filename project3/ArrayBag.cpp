...
**/
template<class ItemType>
void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& another_bag)
{
   for(int i = 0; i < another_bag.getCurrentSize(); i++){         //Use for-loop to add items from the other bag
      if(item_count_ + 1 <= DEFAULT_CAPACITY){                    //Check to see if the Arraybag have enough space to add one more item
         if(!(contains(another_bag.items_[i]))){                  //If the Arraybag does NOT contain the current item from the other Arraybag
            add(another_bag.items_[i]);                           //Add the item
         }
      }
   }
}
