...
void Barbarian::eatTaintedStew() 
{
    ...
    if(getRace() == "UNDEAD"){                  //If the character is undead
        current_vitality += 3;                  //Add 3 to current vitality, nothing else happened
    }
    else{
        current_vitality = 1;                    //If the character is not undead, set the vitality to 1
        toggleEnrage();                          //Set the Character's enraged to the opposite it was
        if(getEnrage() == true){                 //if Character's is now enraged, set the offhand weapon to table
            setSecondaryWeapon("TABLE");
        }
        else{                                    //if character's is not enraged. set the main weapon to bucket
            setMainWeapon("BUCKET");
        }
    }
    setVitality(current_vitality);              //set character's current vitality
