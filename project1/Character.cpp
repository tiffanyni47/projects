...
Character::Character(std::string name, std::string race, int vitality, int armor, int level, bool enemy){  //Parameterized constructor
    ...
    vitality_ = 0; setVitality(vitality);    //Character's vitality, with default value 0
    armor_ = 0; setArmor(armor);             //Character's max armor level, with default value 0
    level_ = 0; setLevel(level);             //Character's level, with default value 0
    enemy_ = false; if(enemy == true){       //A flag indicating whether a character is enemy, default value false
        setEnemy();
    }
}
